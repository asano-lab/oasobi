...
def create_connection_dict(prob_d):
    ...
    while len(prob_d_cp) > 1:
        ...
        # 小数第2位まで有効
        prob_d_cp[next_key] = round(
            sum(v for k, v in prob_d_cp.items() if k in min_syms), 2)
        ...
        for j in range(i, 0, -1):
            ...
            if pattern in known_patterns:
                if i + j >= len(symbol_series):
                    ...
                    print(i, j, pattern)
                    codes.append(
                        known_patterns[pattern] + (symbol_series[i+j],))
                    i += j + 1
                break
        else:
...
def solve_q4_3_2(m):
    ...
    ans = ""
    for i in codes:
        ans += format(i[0], "0%db" % offset_bit) + format(i[1],
                                                          "0%db" % length_bit) + str((i[2] == "B") * 1) + " "
    ans = ans[:-1]
    print(ans)
    return ans
...
def solve_q4_3_3(m):
    ...
    codes = [(int(i[:offset_bit], 2), int(i[offset_bit:offset_bit +
              length_bit], 2), chr(ord("A") + int(i[-1]))) for i in codes]
    print(codes)
...
def solve_q7_1_2(m):
    ...
    for i in range(0, len(codewords), 2):
        hamming_distances.append(
            sum(j == "1" for j in format(codewords[i] ^ codewords[i + 1], "b")))
...
def solve_q7_2_2(m):
    ...
    # 全ての入力
    all_info_series = np.matrix(
        [[int(b) for b in format(i, "0%db" % k)] for i in range(1 << k)])
...
QUESTIONS = {
    "1": {
        ...
        "questions": [
            ...
            {"pattern": re.compile(r'次の情報のうちディジタルがアナログより'),
             "solver": solve_q1_1_3},
            {"pattern": re.compile(r'最高周波数が (\d+) Hz'),
             "solver": solve_q1_1_4},
            {"pattern": re.compile(
                r'送信できるのは、(\d+) Mb/s である.*\n(\d+)レベル'), "solver": solve_q1_1_5},
            {"pattern": re.compile(
                r'ディジタル化するとき, (\d+) ビット/サンプル'), "solver": solve_q1_1_6},
    ...
    "2": {
        ...
        "questions": [
            {"pattern": re.compile(
                r'さいころを 2 回振る。このとき \d の目が 1 回も出ない'), "solver": solve_q2_1_1},
            {"pattern": re.compile(
                r'2 個のさいころを同時に振る。このとき \d の目が出ない'), "solver": solve_q2_1_2},
            {"pattern": re.compile(r'\d の目がちょうど 3 回'), "solver": solve_q2_1_3},
            {"pattern": re.compile(
                r'このとき 1 の目か 2 の目または 1 と 2 の両方'), "solver": solve_q2_1_4},
            {"pattern": re.compile(
                r'\d の目が出た、このとき 2 回目に \d の目'), "solver": solve_q2_1_5},
            {"pattern": re.compile(r'奇数であるとき 1 の目が 1 回'),
             "solver": solve_q2_1_6},
            {"pattern": re.compile(r'大きい方から小さい方を引く'), "solver": solve_q2_1_7},
            {"pattern": re.compile(
                r'2 元対称通信路を用いて P\(X=0\|Y=1\)'), "solver": solve_q2_1_8},
            {"pattern": re.compile(
                r'2 元対称通信路を用いて P\(X=1\|Y=0\)'), "solver": solve_q2_1_9},
            {"pattern": re.compile(
                r'2 元対称通信路を用いて P\(X=1\|Y=1\)'), "solver": solve_q2_1_10}
        ]
    },
    "3-1": {
        ...
        "questions": [
            {"pattern": re.compile(
                r'記号が｛([A-Z ]+)｝の場合'), "solver": solve_q3_1_1},
            ...
            {"pattern": re.compile(
                r'ある記号の発生確率が｛([0-9\.]+)｝の場合'), "solver": solve_q3_1_5},
            {"pattern": re.compile(
                r'</th></tr> <tr> <td>A</td><td>0.05</td><td>00 <br>'), "solver": solve_q3_1_6},
            {"pattern": re.compile(
                r'</th></tr> <tr> <td>A</td><td>0.1 </td><td>000 <br>'), "solver": solve_q3_1_7},
            {"pattern": re.compile(
                r'</th></tr> <tr> <td>A</td><td>0.5 </td><td>1 <br>'), "solver": solve_q3_1_8}
        ]
    },
    "3-2": {
        ...
        "questions": [
            {"pattern": re.compile(
                r'４元情報源の記号発生確率が｛ ([0-9\. ]+) ｝'), "solver": solve_q3_2_1},
            {"pattern": re.compile(
                r'５元情報源の記号発生確率が｛([0-9\. ]+)｝'), "solver": solve_q3_2_2},
            {"pattern": re.compile(r'下記符号の平均符号長'), "solver": solve_q3_2_3},
            {"pattern": re.compile(
                r'３元情報源の記号発生確率が｛ ([0-9\. ]+) ｝'), "solver": solve_q3_2_4}
        ]
    },
    "4-1": {
        ...
        "questions": [
            {"pattern": re.compile(
                r'確率はそれぞれ \{([0-9\. ]+)\}'), "solver": solve_q4_1_1},
            {"pattern": re.compile(
                r'\{a0-&gt;([01]+),a1-&gt;([01]+),a2-&gt;([01]+),a3-&gt;([01]+),a4-&gt;([01]+)\} .*\n.*\{([01]+)\}'), "solver": solve_q4_1_3},
        ]
    },
    "4-2": {
    ...
    "6": {
        ...
        "questions": [
            {"pattern": re.compile(
                r'pが ([0-9\.]+) の場合、通信路容量'), "solver": solve_q6_1_1},
            {"pattern": re.compile(
                r'それぞれ \{([0-9\.]+) ([0-9\.]+) ([0-9\.]+)\} の場合'), "solver": solve_q6_1_2},
            {"pattern": re.compile(
                r'それぞれ \{([0-9\.]+) ([0-9\.]+) ([0-9\.]+)\} である'), "solver": solve_q6_1_3},
            {"pattern": re.compile(
                r'誤る確率pが ([0-9\.]+) の場合'), "solver": solve_q6_1_4}
        ]
    },
    "7-1": {
    ...
    "7-2": {
        ...
        "questions": [
            ...
            {"pattern": re.compile(r'示す。((.*\n)+).*n,k,dmin'),
             "solver": solve_q7_2_2},
            {"pattern": re.compile(
                r'受信した符号語は \{([01 ]+)\}'), "solver": solve_q7_2_4},
        ]
    },
    "7-3": {
